...
describe('Feature: ai-model-proxy, Property 7: RESTful设计合规性', () => {
  ...
  describe('HTTP方法使用', () => {
    ...
    it('GET方法应当用于获取资源', async () => {
      ...
      const statusResponse = await request(app)
        .get(`/api/v1/models/${testJobId}/status`);
      
      const resultResponse = await request(app)
        .get(`/api/v1/models/${testJobId}/result`);
    ...
    it('不应当接受不适当的HTTP方法', async () => {
      ...
      const putResponse = await request(app)
        .put(`/api/v1/models/${testJobId}/status`);
      
      const deleteResponse = await request(app)
        .delete(`/api/v1/models/${testJobId}`);
  ...
  describe('响应头一致性', () => {
    ...
    it('错误响应应当包含适当的安全头', async () => {
      const response = await request(app)
        .get('/api/v1/models/invalid-job-id/status');
      ...
      fc.assert(fc.property(
        ...
        (statusCode) => {
          ...
          if (statusCode >= 200 && statusCode < 300) {
            ...
            // 4xx 客户端错误
            expect([400, 401, 403, 404, 405, 413, 415, 422, 429]).toEqual(expect.arrayContaining([expect.any(Number)]));
